...
class LightNet_from_ft(nn.Module):
    ...
    def forward(self,x):
        ...
        return ground_map.view(-1, 3, 16, 32), sun_map.view(-1, 3, 3, 9), sun_intensity.view(-1, 3)
...
class LightDirectNet(nn.Module):
    ...
    def __init__(self, num_group=1):
        ...
        self.conv1 = nn.Sequential(
                        ...
                        nn.PReLU(),
                        nn.MaxPool2d(3, stride=2, padding=1)) # 128

        self.conv2 = nn.Sequential(
                        ResBlock(64, 64, 3, 1, num_group),
                        nn.PReLU())

        self.conv3 = nn.Sequential(
                        ResBlock(64, 128, 3, 2, num_group),
                        nn.PReLU())  # 64

        self.conv4 = nn.Sequential(
                        ResBlock(128, 128, 3, 1, num_group),
                        nn.PReLU())    

        self.conv5 = nn.Sequential(
                        ResBlock(128, 256, 3, 2, num_group),
                        nn.PReLU())  # 32

        self.conv6 = nn.Sequential(
                        ResBlock(256, 256, 3, 1, num_group),
                        nn.PReLU())    

        self.conv7 = nn.Sequential(
                        ResBlock(256, 512, 3, 2, num_group),
                        nn.PReLU())  # 16

        self.conv8 = nn.Sequential(
                        ResBlock(512, 512, 3, 1, num_group),
                        nn.PReLU()) 


        self.sun_conv1 = nn.Sequential(
                        ResBlock(512, 512, 3, 2, num_group),
                        nn.PReLU())  # 8
        self.sun_conv2 = nn.Sequential(
                        ResBlock(512, 512, 3, 1, num_group),
                        nn.PReLU())  

        self.sun_dense = nn.Sequential(
                        nn.Linear(4*6*512, 512*3),
                        nn.Sigmoid()) 
...
class LightNet_Two_Part_sh(nn.Module):
    ...
    def __init__(self, num_group=1):
        ...
        self.ground_para_dense1 = nn.Sequential(
                        nn.Linear(8*12*512, 512),
                        nn.PReLU()) 
        self.ground_para_dense2 = nn.Sequential(
                        nn.Linear(512, 27)) 
        ...
        self.sun_para_dense1 = nn.Sequential(
                        nn.Linear(4*6*512, 512),
                        nn.PReLU()) 
        self.sun_para_dense2 = nn.Sequential(
                        nn.Linear(512, 1),
                        nn.ReLU()) 
        self.sun_dense = nn.Sequential(
                        nn.Linear(4*6*512, 512*3),
...
class LightNet_Two_Part_sh_single(nn.Module):
    ...
    def __init__(self, num_group=1, input_channel=4):
        ...
        self.sun_para_dense2 = nn.Sequential(
                        nn.Linear(512+27, 3),
                        nn.ReLU())  
        self.sun_para_dense3 = nn.Sequential(
                        nn.Linear(512+27, 1),
                        nn.Sigmoid()) 
        self.sun_dense = nn.Sequential(
                        nn.Linear(4*6*512+512, 512),
                        nn.LogSoftmax(dim=1)) 
    ...
    def getEnvLightConf(self, shlight):
        ...
        Intensity = torch.sum(torch.clamp(envlight, 0., float('inf')), dim=1).reshape(envlight.shape[0], -1)
    ...
    def forward(self, human, mask, gth_ground_shlight=None, crop_mask=None):
        ...
        if gth_ground_shlight is None:
            sun_para = self.sun_para_dense2(torch.cat([dense_feature, ground_shlight], dim=1))
        else:
            sun_para = self.sun_para_dense2(torch.cat([dense_feature, gth_ground_shlight.reshape(gth_ground_shlight.shape[0], -1)], dim=1))

        
        if gth_ground_shlight is None:
            sun_threshold = self.sun_para_dense3(torch.cat([dense_feature, ground_shlight], dim=1))
        else:
            sun_threshold = self.sun_para_dense3(torch.cat([dense_feature, gth_ground_shlight.reshape(gth_ground_shlight.shape[0], -1)], dim=1))
        ...
        est_position = torch.sum((coord_tensor * torch.exp(sun)).reshape(sun.shape[0], 2, -1), axis=2)

        return ground_shlight, sun, sun_para, env_conf.reshape(env_conf.shape[0], 1, 16, 32), est_position, sun_threshold.reshape(-1)
...
def getEnvTransport():
    ...
    for i in range(hdr_height):
        for j in range(hdr_width):
            angles[i, j] = [i/hdr_height*np.pi, 0.5*np.pi-j/hdr_width*np.pi*2 ]
    ...
    for i in range(hdr_height):
        for j in range(hdr_width):
            theta = angles[i, j, 0]
            phi = angles[i, j, 1]
            normal[i, j] = [np.cos(phi) * np.sin(theta), np.sin(phi)*np.sin(theta), np.cos(theta)]
...
def getTransportMap(normal):
    ...
    transport_map[:,:,4] = 3 * normal[:,:,2] * normal[:,:,2] - 1.
    transport_map[:,:,5] = normal[:,:,0] * normal[:,:,1]
    transport_map[:,:,6] = normal[:,:,0] * normal[:,:,2]
    transport_map[:,:,7] = normal[:,:,1] * normal[:,:,2]
    transport_map[:,:,8] = normal[:,:,0] * normal[:,:,0] - normal[:,:,1] * normal[:,:,1]

    return transport_map
    
env_transport_tensor = getEnvTransport()
env_transport_tensor = torch.FloatTensor(np.expand_dims(np.expand_dims(env_transport_tensor.transpose(2, 0, 1), 0), 0)).cuda()
...
class LightNet_Hybrid(nn.Module):
    ...
    def __init__(self, num_group=1, input_channel=4):
        ...
        self.human_conv2 = nn.Sequential(
                        ResBlock(64, 64, 3, 1, num_group),
                        nn.PReLU())
    ...
    def forward(self, human, mask, gth_ground_shlight=None, crop_mask=None):
        if crop_mask is None:
            ...
            bg_f = self.encode_bg(torch.cat([human*(1.-mask), mask], dim=1))
        else:
            all_f = self.encode_human_bg(torch.cat([human, mask, crop_mask], dim=1))
            human_f = self.encode_human(torch.cat([human*mask, mask, crop_mask], dim=1))
            bg_f = self.encode_bg(torch.cat([human*(1.-mask), mask, crop_mask], dim=1))


        ground_shlight, sun, sun_para = self.decode(all_f, gth_ground_shlight)
        human_ground_shlight, human_sun, human_sun_para = self.decode(human_f, gth_ground_shlight)
        ...
        ground_threshold = self.ground_threshold_dense(torch.cat([all_f, human_f, bg_f], dim=1))
        sun_threshold = self.sun_map_threshold_dense(torch.cat([all_f, human_f, bg_f], dim=1))
        sun_intensity_threshold = self.sun_intensity_threshold_dense(torch.cat([all_f, human_f, bg_f], dim=1))


        return ground_shlight, sun, sun_para, \
                human_ground_shlight, human_sun, human_sun_para, \
                bg_ground_shlight, bg_sun, bg_sun_para, \
                ground_threshold, sun_threshold, sun_intensity_threshold                    
...
class AlexNet_Single_SH(nn.Module):
    ...
    def forward(self, human, mask, nothing, crop_mask=None):
        ...
        return ground_shlight, sun, sun_para, None, est_position, sun_threshold.reshape(-1)
...
class LightNet(nn.Module):
    ...
    def __init__(self, input_channel=4):
        ...
        self.conv2 = nn.Sequential(
                        ResBlock(64, 64, 3, 1, 4),
                        nn.PReLU())

        self.conv3 = nn.Sequential(
                        ResBlock(64, 128, 3, 2, 8),
                        nn.PReLU())  # 64

        self.conv4 = nn.Sequential(
                        ResBlock(128, 128, 3, 1, 8),
                        nn.PReLU())    

        self.conv5 = nn.Sequential(
                        ResBlock(128, 256, 3, 2, 16),
                        nn.PReLU())  # 32

        self.conv6 = nn.Sequential(
                        ResBlock(256, 256, 3, 1, 16),
                        nn.PReLU())    

        self.conv7 = nn.Sequential(
                        ResBlock(256, 512, 3, 2, 16),
                        nn.PReLU())  # 16

        self.conv8 = nn.Sequential(
                        ResBlock(512, 512, 3, 1, 16),
                        nn.PReLU()) 

        self.ground_conv1 = nn.Sequential(
                        ResBlock(512, 512, 3, 2, 16),
                        nn.PReLU())  # 16
        self.ground_conv2 = nn.Sequential(
                        ResBlock(512, 512, 3, 1, 16),
                        nn.PReLU())  
