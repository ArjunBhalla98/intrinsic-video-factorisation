...
class BilateralLayer(nn.Module):
    def __init__(self, mode = 0, isCuda = True):
        ...
        if mode == 0:
            self.conv1 = nn.Conv2d(in_channels = 9, out_channels=16, kernel_size=4, stride = 2, bias=True)
        else:
            print('invalid mode')
            exit(0)
...
class BilateralSolver:
    ...
    def apply(self, image, pred, confidence):
        ...
        for i in range(batch_size):
            ...
            curr_result, yhat = bs.solve(grid, curr_pred, curr_conf, self.bs_params, im_shape)
            output[i, :, :, :] = curr_result
            #  print yhat.shape
