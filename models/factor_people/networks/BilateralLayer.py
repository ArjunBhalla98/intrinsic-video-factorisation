...
from . import BilateralGrid as bs
...
class BilateralFunction(torch.autograd.Function):
    ...
    def forward(ctx, image, pred, confidence, grid_params_arr, bs_params_arr):
        ...
        image_np = image.cpu().swapaxes(1, 2).swapaxes(2, 3)
        pred_np = pred.cpu().swapaxes(1, 2).swapaxes(2, 3)
        conf_np = confidence.cpu().squeeze(1)
    ...
    def backward(ctx, grad_output, grad_not_used):
        ...
        image_np = image.data.cpu().swapaxes(1, 2).swapaxes(2, 3)
        pred_np = pred.data.cpu().swapaxes(1, 2).swapaxes(2, 3)
        conf_np = confidence.data.cpu().squeeze()
        grad_output_np = grad_output.data.cpu().swapaxes(1, 2).swapaxes(2, 3)
...
class BilateralLayer(nn.Module):
    def __init__(self, mode=0, isCuda=True):
        ...
        if mode == 0:
            self.conv1 = nn.Conv2d(
                in_channels=9, out_channels=16, kernel_size=4, stride=2, bias=True
            )
        else:
            print("invalid mode")
            exit(0)
    ...
    def forward(self, input, feature, pred):
        ...
        scale, _ = torch.max(scale, dim=3, keepdim=True)
        scale = torch.clamp(scale, 1e-5, 1)
        feature = feature / scale.expand_as(feature)
...
class BilateralSolver:
    ...
    def apply(self, image, pred, confidence):
        ...
        for i in range(batch_size):
            ...
            grid = bs.BilateralGrid(curr_image * 255.0, **self.grid_params)

            curr_result, yhat = bs.solve(
                grid, curr_pred, curr_conf, self.bs_params, im_shape
            )
            output[i, :, :, :] = curr_result
            #  print yhat.shape
