...
def Uncharted2ToneMapping_tensor(color, adapted_lum):
    WHITE = F(11.2)
    tonemapping_img = F(1.6 * adapted_lum * color) / WHITE
    tonemapping_img = (torch.clamp(tonemapping_img, min=0.) + 1e-7) ** (1/2.2)
    return tonemapping_img

def tonemapping(input):

    return np.clip(Uncharted2ToneMapping(input, 1.), 0., 1.)


def tonemapping_tensor(input):

    return torch.clamp(Uncharted2ToneMapping_tensor(input, 1.), 0., 1.)
...
def DSSIM(x, y, mask=None):
    ...
    return 0.5 * (1. - ssim(x, y, data_range=max_range, multichannel=True))
...
def savefig(img, path):
    ...
    Image.fromarray((np.clip(save_img* 255, 0., 255.) ).astype(np.uint8)).save(path)
...
def recoverySH(shlight):
    ...
    return np.clip(getShading(env_transport_map, shlight), 0., float('inf'))
...
def recoveryEnvLight(ground, sun_map_exp, sun_intensity):
    ...
    sun_map_exp = sun_map_exp / torch.max(sun_map_exp.reshape(sun_map_exp.shape[0], -1), dim=1)[0].reshape(sun_map_exp.shape[0], 1, 1, 1)
    ground_map = recoverySH_Tensor(ground)
    light_full = ground_map + sun_map_exp * sun_intensity.reshape(sun_intensity.shape[0], 3, 1, 1)
    ...
    if scene is not None:
        if not (scene.shape[0] == 768 and scene.shape[1] == 512):
            if scene.shape[1] * 1.5 > scene.shape[0]:
                ...
                scene = cv2.resize(scene[:,padding:scene.shape[1]-padding], (512, 768), interpolation=cv2.INTER_AREA)
            else:
                scene = cv2.resize(scene[scene.shape[0]-int(scene.shape[1]*1.5):,:], (512, 768), interpolation=cv2.INTER_AREA)
...
def GetSceneList(Scene_img, human_num, offset=0):
    ...
    resized_scene = cv2.resize(Scene_img, (resized_width, 768), interpolation=cv2.INTER_AREA)
    ...
    for i in range(human_num):
        ...
        img[:,:,:] = resized_scene[:,offset_begin+i*interval:offset_begin+i*interval+512,:]
        img_list.append(img)
...
def CompositeScene(scene, img_list, interval, offset_begin, offset_end):
    ...
    target[:,offset_begin:offset_begin+begin_length] = img_list[0][:,0:begin_length]
    target[:,width-offset_end-(img_width-end_length):width-offset_end] = img_list[-1][:,-(img_width-end_length):]

    for i, img in enumerate(img_list):
        target[:,offset_begin + i * interval + begin_length:offset_begin + i * interval + end_length] = img[:, begin_length:end_length]
...
def composition(albedo, shading, mask, bg, shadow):
    return (tonemapping_tensor(albedo*shading) * mask + shadow * bg * (1.-mask)).contiguous()

def composition_noshadow(albedo, shading, mask, bg):
    return (tonemapping_tensor(albedo*shading) * mask + bg * (1.-mask)).contiguous()
...
class ImageCompositor:
    ...
    def __init__(self, row_num = 2, col_num = 5, img_shape=(768, 512)):
        ...
        self.img = np.zeros((row_num * (img_shape[0]+self.row_interval) + self.row_interval, col_num * (img_shape[1]+self.col_interval) + self.col_interval, 3), np.uint8)
